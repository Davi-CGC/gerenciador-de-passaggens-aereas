...
function atualizarVisorPrecoPublico() {
    const visor = document.getElementById('total-preco-acumulado');
    if (!visor) return;
    
    // Soma o preço de todos os assentos guardados no carrinho temporário
    let total = assentosSelecionadosNestaCompra.reduce((soma, assento) => soma + assento.preco, 0);
    
    // Atualiza o texto na tela formatado como dinheiro
    visor.innerText = `R$ ${total.toFixed(2)}`;
}